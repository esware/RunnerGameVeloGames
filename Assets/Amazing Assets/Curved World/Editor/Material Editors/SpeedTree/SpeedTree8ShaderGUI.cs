...
namespace AmazingAssets.CurvedWorldEditor
{
    class SpeedTree8ShaderGUI : ShaderGUI
    {
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            {
                MaterialProperty property5 = ShaderGUI.FindProperty("_SubsurfaceIndirect", properties);
                materialEditor.ShaderProperty(property5, SpeedTree8ShaderGUI.Styles.subsurfaceIndirectText, 2);
            }
            MaterialProperty property10 = ShaderGUI.FindProperty("_BillboardKwToggle", properties);
            SpeedTree8ShaderGUI.MakeAlignedProperty(property10, SpeedTree8ShaderGUI.Styles.billboardText, materialEditor, true);
            if ((double)property10.floatValue > 0.0)
            {
                MaterialProperty property5 = ShaderGUI.FindProperty("_BillboardShadowFade", properties);
                materialEditor.ShaderProperty(property5, SpeedTree8ShaderGUI.Styles.billboardShadowFadeText, 2);
            }
        ...
        {
            Rect controlRect = EditorGUILayout.GetControlRect(true, UnityEditor.EditorGUIUtility.singleLineHeight + 2f);
            controlRect.width = UnityEditor.EditorGUIUtility.labelWidth + UnityEditor.EditorGUIUtility.fieldWidth * (doubleWide ? 2f : 1f);
        ...
        {
            Rect controlRect = EditorGUILayout.GetControlRect(true, UnityEditor.EditorGUIUtility.singleLineHeight + 2f);
            controlRect.width = UnityEditor.EditorGUIUtility.labelWidth + UnityEditor.EditorGUIUtility.fieldWidth / 2f;
            ...
            {
                controlRect.width = UnityEditor.EditorGUIUtility.labelWidth + UnityEditor.EditorGUIUtility.fieldWidth * (doubleWide ? 2f : 1f);
                controlRect.x += UnityEditor.EditorGUIUtility.fieldWidth / 2f;
